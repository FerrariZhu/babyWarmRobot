...
-- 登录: demo_user_1@baby-outfit.dev ~ demo_user_50@baby-outfit.dev
-- 密码: password123
-- ============================================================
...
where user_id in (
  ...
  where email like 'demo_user_%@baby-outfit.dev'
...
from auth.users u
where u.email like 'demo_user_%@baby-outfit.dev';
...
cross join lateral (
  ...
  where u.email = 'demo_user_' || ((gs.i - 1) % 50 + 1) || '@baby-outfit.dev'
  limit 1
) sub
cross join lateral (
  select id, code from public.materials
...
cross join lateral (
  ...
  where u.email = 'demo_user_' || i || '@baby-outfit.dev'
  limit 1
) sub;
...
from generate_series(1, 50) as i
join auth.users u on u.email = 'demo_user_' || i || '@baby-outfit.dev';
...
begin
  ...
  raise notice 'Demo 账号: demo_user_1@baby-outfit.dev ~ demo_user_50@baby-outfit.dev，密码 password123';
end $$;
