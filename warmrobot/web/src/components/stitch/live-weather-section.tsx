...
import type { WeatherResult } from "@warmrobot/core";
...
export function LiveWeatherSection({
  ...
  // 已有服务端天气时不自动调 GPS（桌面端常超时）；用户下拉刷新或点按钮时再定位
  useEffect(() => {
  ...
  if (!weather) {
    return (
      <section className="rounded-xl bg-secondary-fixed p-6 text-center">
        <MaterialIcon name="cloud_off" className="mb-3 text-[40px] text-on-secondary-fixed" />
        <p className="font-body-md text-on-secondary-fixed">
          {errorMessage ?? "无法获取天气，请检查定位权限与网络。"}
        </p>
        <button
          type="button"
          onClick={() => void syncFromDevice({ refreshPage: true })}
          className="font-label-caps mt-4 inline-flex min-h-touch-target-min items-center gap-2 rounded-full bg-primary px-6 py-3 text-on-primary cloud-shadow"
        >
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-3">
        ...
        {errorMessage && (
          <div className="flex flex-col items-center gap-2 rounded-xl bg-secondary-fixed px-4 py-3 text-center">
            <p className="font-body-md text-on-secondary-fixed">{errorMessage}</p>
            <p className="font-label-caps text-on-secondary-fixed/80">上方显示的是已保存或默认城市天气</p>
            <button
              type="button"
              onClick={() => void syncFromDevice({ refreshPage: true })}
              className="font-label-caps inline-flex min-h-touch-target-min items-center gap-1 text-on-secondary-fixed underline-offset-2 hover:underline"
            >
